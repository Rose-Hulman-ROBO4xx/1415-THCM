...
void loop(){
    ...
    // Increment the count quite quickly
    if (count < 1000){
        count++;
    }
    // When the count hits 1000 increment the speed.  Happens at ~1s intervals
    else{
        count = 0;
      if(speedval>999){
        speedval=0;
    ...
    // Since the count is changing quickly this will appear to write all three digits at once
    if(count%2==0){
      digit=2;
    }
    else if(count%3==0){
